...
ARQUIVO_ENTRADA = 'Tabelas/media_acumulada_ponderada_ffill.csv' 
SEMESTRES_FUTUROS = 3
ARQUIVO_SAIDA = 'Tabelas/previsao_media_acumulada_ponderada_SARIMAX.csv'

# Parâmetros SARIMAX (os mesmos usados anteriormente)
SARIMAX_ORDER = (1, 1, 0)
...
def rodar_previsao_sarimax_media(df_tabela, order, seasonal_order):
    """Aplica o SARIMAX coluna por coluna para séries contínuas (Média Acumulada Ponderada)."""
    ...
    for curso_id in tabela_data_only.columns:
        ...
        if pd.isna(primeiro_valor_valido):
            print(f"Aviso: O curso {curso_id} não possui dados válidos para previsão de Média Acumulada.")
            continue
        ...
        try:
            ...
            # Não aplicamos restrição a zero. Mantemos 2 casas decimais.
            yhat = yhat.round(2)
    ...
    print("\n" + "="*90)
    print(f"✅ SUCESSO! Previsão de Média Acumulada Ponderada com SARIMAX salva em '{ARQUIVO_SAIDA}'")
...
try:
    ...
    rodar_previsao_sarimax_media(df_input, SARIMAX_ORDER, SARIMAX_SEASONAL_ORDER)
